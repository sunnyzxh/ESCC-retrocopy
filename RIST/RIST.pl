...
#intronic TMEM132B chr12 125801102 125801147 Germline chr12 104359592 104382656 104359629 104382442 TDG FJ1705100058LD01.sorted.markdup.bqsr
        ...
        if ( $#insert == -1 ) {
            ...
            $ins_e = $ins_tmp[1];
            if ( $ins_e - $ins_s + 1 < 60 ) {
                $seq = `$samtools faidx $fa $ins_chr:$ins_s-$ins_e |tail -1`; chomp $seq;
            }
        }
        ...
        if ( !$ref_genes{$info[7]} ) {
            $n_junction = "NA";
            my $seq0 = `$samtools faidx $fa $info[3]:$info[4]-$info[5]`; chomp $seq0;
            my @seqs = split /\n/, $seq0;
            for my $s ( 1..$#seqs ) {
            ...
            for my $i ( $start_exon..$end_exon ) {
                my $seq0 = `$samtools faidx $fa $trans_d[2]:$exon_s[$i]-$exon_e[$i]`; chomp $seq0;
                my @seqs = split /\n/, $seq0;
                for my $s ( 1..$#seqs ) {
                    $retro_seq = "$retro_seq"."$seqs[$s]";
                }
